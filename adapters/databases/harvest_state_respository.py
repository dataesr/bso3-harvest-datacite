    ...
    def update(self, elements_updated: dict):
        ...
        for key in keys_arg:
            if key not in harvest_state_table_attributes:
                raise Exception(
                    f"Element to update not available in HarvestStateTable : '{key}'")
            ...
            if type_element_to_update_arg != type_element_to_update_expected:
                raise Exception(
                    f"Element type to update not correct for HarvestStateTable : got '{type_element_to_update_arg}' and expected '{type_element_to_update_expected}' for '{key}' attribute")
