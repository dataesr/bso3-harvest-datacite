...
def get_list_creators_or_contributors_and_affiliations(path_file, number_of_processed_dois):
        ...
        if jsonstring.strip() != "":
            ...
            if json_obj["data"]:
                for idx, doi in enumerate(json_obj["data"]):
                    ...
                    try:
                        list_creators_or_contributors_and_affiliations += concat_affiliation(
                            doi, "creators"
                        )
                        list_creators_or_contributors_and_affiliations += concat_affiliation(
                            doi, "contributors"
                        ...
                        print(f"Error during concat_affiliation\n{doi}", e)

                    if len(list_creators_or_contributors_and_affiliations) > current_size:
                        number_of_non_null_dois += 1
                    else:
                        number_of_null_dois += 1
                number_of_processed_dois += idx + 1

    print(
        f"{path_file} {number_of_processed_dois} dois, {number_of_non_null_dois} non-null, {number_of_null_dois} null"
    )
    return number_of_processed_dois, list_creators_or_contributors_and_affiliations
...
def split_dump_file_concat_and_save_doi_files(
    ...
    for i, path_file in enumerate(list_path):
        print(f"file {i}/{len(list_path)}")
        number_of_processed_dois, list_creators_or_contributors_and_affiliations = get_list_creators_or_contributors_and_affiliations(path_file, number_of_processed_dois)
